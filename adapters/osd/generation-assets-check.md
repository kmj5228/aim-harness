# Generation Assets Check: osd

## Purpose

Verify that `adapters/osd/mappings.yaml` `generation_assets` entries match the current generated `osd-harness` tree.
...
- these are valid generated assets for the current `osd` draft
- but they come from `harness-initiator` default base-runtime carry-over policy rather than from template source-asset mapping
- this carry-over is expected as long as the default carry-over policy stays in the initiator skill

## Conclusion

- `generation_assets` explains the template-derived portion of the current `osd` generation pass
- base-runtime carry-over is still better explained by initiator default policy than by another adapter schema layer
