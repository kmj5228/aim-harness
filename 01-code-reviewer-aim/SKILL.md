...
description: "AIM 프로젝트 코드 리뷰 자동화 파이프라인. IMS/Jira/GitLab 정보 수집 → 리뷰 Plan 생성 → 코드/테스트/커버리지 리뷰 → 종합 보고서 → GitLab MR 코멘트 등록까지 자동화한다. '코드 리뷰', 'code review', '리뷰', 'review', 'AIM 코드 리뷰', 'AIM 리뷰', 'code-reviewer-aim' 등으로 트리거한다. C 언어/AIM 모듈 규칙에 특화되어 있다."
---
...
- 예: `../agent/review/<topic>/02_code_review.md` → `/Users/mjkang/company/dev_sshfs/agent/review/<topic>/02_code_review.md`
...
수행:
1. `../agent/review/<topic>/` 디렉토리 생성
2. 입력 정리하여 `../agent/review/<topic>/00_input.md` 저장
...
1. `01_info_collection.md` 기반으로 `../agent/review/<topic>/<topic>_review.plan.md` 생성
...
**Phase C gate**: `../agent/review/<topic>/<topic>_review.plan.md` 파일이 존재하고, 사용자가 승인(또는 --auto)해야 Phase D 진행.
...
| 에이전트 | 산출물 |
|---------|--------|
| aim-code-reviewer | `../agent/review/<topic>/02_code_review.md` |
| aim-test-reviewer | `../agent/review/<topic>/03_test_review.md` |
| aim-coverage-analyst | `../agent/review/<topic>/04_coverage.md` |
...
1. `../agent/review/<topic>/01_info_collection.md`
2. `../agent/review/<topic>/02_code_review.md`
3. `../agent/review/<topic>/03_test_review.md`
4. `../agent/review/<topic>/04_coverage.md`
...
모든 산출물은 `../agent/review/<topic>/` 하위에 저장한다.
...
- 올바른 예: `../agent/review/acsapi_aix_351005_6293/02_code_review.md`
