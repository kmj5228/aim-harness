...
본 에이전트는 **main aim 경로(`/Users/mjkang/company/dev_sshfs/aim/` = `/root/ofsrc/aim`)에서만 측정**한다. 오케스트레이터가 다른 에이전트(info/code/test/synthesizer)를 worktree에서 실행하더라도, 커버리지 측정은 반드시 main aim에서 수행한다.

근거: `aim/AGENTS.md` "Worktree Operations" 운영 규칙 — 워크트리는 `$OPENFRAME_HOME` install 산출물과 분리되어 측정 일관성을 보장하기 어렵다 (`measure_diff_cov.sh`도 main aim 기준 동작).
