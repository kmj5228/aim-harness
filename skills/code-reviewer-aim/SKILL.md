...
오케스트레이터는 Phase B~F의 모든 spawn prompt에 `WORKSPACE_AIM` 경로를 명시한다 (예: "작업 경로: <WORKSPACE_AIM>"). 단 **aim-coverage-analyst는 main aim에서 측정**(governance 규칙) — `aim/AGENTS.md` "Worktree Operations" 운영 규칙 준수.

운영 규칙(install/서버기동/측정 금지)과 셋업 절차 상세는 SSoT 참조:
...
**작업 경로 주입(필수)**: 각 spawn prompt에 Phase A에서 결정된 `WORKSPACE_AIM` 경로를 명시한다 (예: "작업 경로: <WORKSPACE_AIM>"). 단 **aim-coverage-analyst는 main aim 경로(`/Users/mjkang/company/dev_sshfs/aim/`)를 명시**한다. governance 규칙상 커버리지 측정은 worktree에서 수행하지 않는다 (`aim/AGENTS.md` "Worktree Operations" 참조).
...
- 커버리지 측정은 **main aim에서 직렬 수행** (worktree 금지 — `aim/AGENTS.md` "Worktree Operations" 운영 규칙). `make`/`make gtest`는 install 안 하므로 `tmdown` 선행 불필요. 별도 `make install` 시에만 `dx tmdown -y` 선행.
