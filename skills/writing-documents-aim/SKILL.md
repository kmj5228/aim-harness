...
**중립성 검토 (실명 당사자 비교/평가 문서):** 실명 당사자를 비교·평가·분담하는 문서(예: 2인 업무분담, 성과 비교)를 공유 채널에 게시하기 전, **각 당사자 관점 + 중립 감사관의 다관점 적대 검토**로 저자 편향을 잡고 교정한다. **자저작 편향은 스스로 인지하지 못하므로** 독립 검토가 필수다. 사실·데이터는 보존하고 옹호·판단·위계 어조만 제거하며, 한쪽(특히 요청자)에 유리한 새 단정도 배제한다(양방향 균형). 편향 키워드 0건 확인 후 게시.
...
**문체는 플랫폼이 아니라 문장의 기능으로 가른다.**

| 문장의 기능 | 문체 | 예 |
|------------|------|-----|
| **기록** — 사실·판단·계획·절차를 적는다 | **간결체** ("~한다", "~이다") | "김인수 연구원이 aim 모듈을 담당한다." |
| **요청·대화** — 특정 상대에게 부탁하거나 말을 건다 | **격식체** ("~합니다", "~드립니다") | "김인수 연구원님, 검토 부탁드립니다." |

- **한 문서 안에 둘이 섞이는 것은 정상이다** — 기능이 다르기 때문이다.
  본문은 간결체로 적고, 담당자에게 준비물을 부탁하는 대목만 격식체로 쓴다.
- 목록 항목은 명사형 종결("~함", "~음")을 허용한다.
- 메일 본문과 Jira/IMS 댓글은 대개 전체가 대화이므로 사실상 격식체가 된다.
...
| [ ] | 문체를 문장의 기능에 맞게 골랐는가? (기록=간결체, 요청·대화=격식체) |
| [ ] | 플랫폼 가이드의 템플릿을 따랐는가? |
...
- **문체를 문장의 기능이 아니라 습관으로 고름** — 기록 문장에 격식체, 요청 문장에 간결체 (기준: Step 2 톤)
- 반말 사용
