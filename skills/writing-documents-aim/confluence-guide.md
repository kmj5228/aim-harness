...
- **기록(사실·판단·계획·절차)은 간결체**("~한다", "~이다"), **요청·대화는 격식체**("~합니다")
- 한 페이지에 둘이 섞이는 것은 정상 — 기준은 문장의 기능이다 (SSoT: `SKILL.md` Step 2 톤)
- **인사: "안녕하십니까"** (댓글 등)
...
**⚠️ 이미지/첨부 맹점 (텍스트 diff의 한계):** 본문을 태그 strip해 텍스트만 diff하면, 사용자가 UI에서 넣은 `<ac:image>`/`ri:attachment`는 텍스트가 없어 "편집 없음"으로 오판해 드롭된다. 갱신 전 **현재 body의 `<ac:image>`/`ri:filename` 목록을 추출**해 새 body에 전부 포함됐는지 확인한다. 내가 만든 이미지(arch/seq 등) 외의 첨부는 사용자 추가분이다. **전면 재생성보다 현재 body에 타겟 삽입**이 안전하다. 첨부 파일 자체는 본문 참조가 빠져도 페이지에 남으므로 `GET .../child/attachment`로 파일명 확인 후 복원 가능. (사고: OF7 CI/CD 문서에서 사용자가 넣은 성공 캡쳐본을 텍스트-only diff가 못 잡아 드롭 → 지적받음. 무인 게시일수록 PUT 전 이미지 ref 보존 필수 체크.)
