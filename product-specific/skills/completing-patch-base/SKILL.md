...
completing-patch 진입 시 가장 먼저 수행한다. 매뉴얼 작업 진입점이 `finishing-a-development-branch-base`과 이 스킬 둘이라서 중복 실행을 방지하기 위한 상태 기반 분기이다.
...
공통 문서 작성 규칙(독자별 추상화, 톤, 두괄식)은 **writing-documents-base** SKILL.md를 참조한다. 아래는 패치 검증서 고유 규칙만 기술한다.
...
`writing-documents-base/manual-guide.md`의 Step 1(매뉴얼 필요성 판단 GATE)을 지금 실행한다. 판단 결과에 따라:
...
**Called by:**
- **finishing-a-development-branch-base** 이후 (MR merge 후)

**Uses:**
- **writing-documents-base** — 공통 문서 작성 규칙 (독자/톤/두괄식)
- **writing-documents-base / manual-guide.md** — Step 0 marker 확인 + Step 6 매뉴얼 후속 작업
