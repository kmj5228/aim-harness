...
| Situation | Skill |
|-----------|-------|
| Session start or startup contract re-entry | `using-ofgw-harness` |
...
- use `coverage-review` when the touched scope reaches `ofgwSrc` and coverage claims need to stay within the confirmed JaCoCo boundary
...
- `skills/review/code-reviewer/scripts/measure_diff_cov.sh` is an optional repo-native helper that estimates added-line coverage from JaCoCo XML plus git diff; treat it as supplemental evidence only.
