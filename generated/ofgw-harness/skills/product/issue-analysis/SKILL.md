---
name: issue-analysis
description: Use when an ofgw issue, Jira ticket, or IMS report needs triage before deciding whether to explain, configure, or fix
---
...
This generated skill keeps the reusable issue-analysis skeleton but applies `ofgw` bindings:
...
3. Trace the relevant code path in `ofgw`.
...
- Keep `ofgw` stack facts in evidence and examples, not in the verdict model itself.
- Do not hardcode JVM-specific reasoning into the reusable verdict structure.
