...
4. Request self-review first (code-reviewer Phase A~E)
...
MR description 작성 규칙 상세는 **writing-documents**의 gitlab-guide.md를 참조한다.

**MR 생성 직후 — 매뉴얼 필요성 자동 판단 (필수)**:

MR 생성 후 `writing-documents/manual-guide.md`의 Step 1 (필요성 판단 GATE)을 자동 호출한다. 이 단계를 건너뛰지 않는다.
...
5. **Marker 삽입**: MR description 맨 아래에 HTML 주석 marker를 추가한다. **IMS 단위로 기록**. 이는 `completing-patch`이 merge 시점에 상태를 읽기 위함.
...
세부 필드 및 조회 규칙은 `writing-documents/manual-guide.md`의 "Marker 형식" 섹션 참조.
...
**Marker가 없는 MR**은 `completing-patch`이 "판단 생략됨"으로 간주하고 merge 시점에 매뉴얼 판단을 실행한다. 따라서 Option 3(Keep as-is) 같은 경로로 MR을 만들지 않은 상태에서는 marker가 존재하지 않고, 그 경우 completing-patch에서 처리된다.

MR description follows `repository merge-request template`:
...
5. `## Squash Commit Message` — `repository git message template` 전체 포맷 (IMS#, module, version)
...
Invoke requesting-code-review → code-reviewer Phase A~E with `--auto`.
...
| 4. Self-review | done | after review | code-reviewer |
...
**Feeds into:**
- **completing-patch** — MR merge 후 IMS 패치 검증서 작성 + 매뉴얼 marker 확인 후 후속 진행
- **writing-documents / manual-guide.md** — Option 1/2 MR 생성·갱신 직후 Step 1 필요성 판단 자동 호출

**Pairs with:**
- **verification-before-completion** — verify before finishing
- **requesting-code-review** — optional self-review (Option 4)
...
| completing-patch 진입 시 | 대상 IMS marker 없음 | 매뉴얼 판단이 생략된 상태 → 지금 판단 |
| completing-patch 진입 시 | `ims=<X> status=pending-merge` | 해당 IMS는 "필요 + merge 후 진행" 결정됨 → 바로 manual-guide Step 2~8 진행 |
| completing-patch 진입 시 | `ims=<X> status=done` | 해당 IMS는 이미 처리 완료 → skip |
