---
name: code-reviewer
description: "Use when reviewing someone else's AIM merge request or when triggered by requesting-code-review for self-review. Requires MR number and topic name."
---
...
- `agent/` → `agent/`
- 예: `agent/<topic>/02_code_review.md` → `agent/prompt/<topic>/02_code_review.md`
...
수행:
1. `agent/<topic>/` 디렉토리 생성
2. 입력 정리하여 `agent/<topic>/00_input.md` 저장
...
1. `01_info_collection.md` 기반으로 `agent/<topic>/<topic>_review.plan.md` 생성
...
**Phase C gate**: `agent/<topic>/<topic>_review.plan.md` 파일이 존재하고, 사용자가 승인(또는 --auto)해야 Phase D 진행.
...
각 에이전트의 prompt에 `01_info_collection.md`의 내용과 해당 에이전트 정의(`skills/review/code-reviewer/ prompt assets`)를 포함한다.
...
| 에이전트 | 산출물 |
|---------|--------|
| aim-code-reviewer | `agent/<topic>/02_code_review.md` |
| aim-test-reviewer | `agent/<topic>/03_test_review.md` |
| aim-coverage-analyst | `agent/<topic>/04_coverage.md` |
...
1. `agent/<topic>/01_info_collection.md`
2. `agent/<topic>/02_code_review.md`
3. `agent/<topic>/03_test_review.md`
4. `agent/<topic>/04_coverage.md`
...
GitLab MR 코멘트의 톤/구조/심각도 표시 규칙은 **writing-documents**의 gitlab-guide.md "MR 코멘트" 섹션을 참조한다.
...
- 올바른 예: `agent/acsapi_aix_351005_6293/02_code_review.md`
...
**Called by:**
- **requesting-code-review** — 셀프 리뷰 (--auto, Phase A~E만)
...
- 커버리지 스크립트: `skills/review/code-reviewer/scripts/measure_diff_cov.sh`
- 커버리지 측정 전 `dx tmdown -y` 필수 (실행 중 서버가 `Text file busy` 유발)
