...
- `agent/<topic>/01_info_collection.md`의 변경 파일 목록
...
```bash
dx bash -c "bash /root/ofsrc/aim/skills/review/code-reviewer/scripts/measure_diff_cov.sh [BASE_BRANCH]"
```
...
`agent/<topic>/04_coverage.md` 파일로 저장:
...
- 변경 파일 목록:
  - `<path>` (+X lines)
- 측정 스크립트: `skills/review/code-reviewer/scripts/measure_diff_cov.sh`
