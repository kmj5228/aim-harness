...
Treat this directory as the standalone `osd-harness` runtime v1.
...
| Situation | Skill |
|-----------|-------|
| Session start or startup contract re-entry | `using-osd-harness` |
...
| Branch finalization and PR preparation | `finishing-a-development-branch` |
...
- use `coverage-review` when touched scope reaches `src/lib`, `src/server`, `src/tool`, or `src/util` and coverage claims need to stay within the confirmed `test/run_coverage.sh` boundary
...
- `coverage-review` is the `osd` coverage counterpart built around the confirmed `test/run_coverage.sh` command.
